...
let test_map_key_value_types () =
  ...
  List.iter (fun (key_type, value_type, expected_key, expected_value) ->
    let config = make_map_config 1024 () in
    let pos = make_position 1 1 "test.ks" in
    let map_decl = make_ast_map_declaration "test" key_type value_type HashMap config true ~is_pinned:false pos in
...
let test_map_operations () =
  let pos = make_position 1 1 "test.ks" in
  let config = make_map_config 1024 () in
  let map_decl = make_ast_map_declaration "counter" U32 U64 HashMap config true ~is_pinned:false pos in
...
let test_map_initialization () =
  let pos = make_position 1 1 "test.ks" in
  let config = make_map_config 1024 () in
  let map_decl = make_ast_map_declaration "initialized_map" U32 U64 HashMap config true ~is_pinned:false pos in
...
let test_map_validation () =
  ...
  (* Test valid map *)
  let config = make_map_config 1024 () in
  let valid_map = make_ast_map_declaration "valid" U32 U64 HashMap config true ~is_pinned:false pos in
  check string "valid map name" "valid" valid_map.name;
  
  (* Test invalid map with zero size *)
  let invalid_config = make_map_config 0 () in
  let invalid_map = make_ast_map_declaration "invalid" U32 U64 HashMap invalid_config true ~is_pinned:false pos in
...
let test_map_program_integration () =
  ...
  (* Create maps *)
  let packet_count_map = make_ast_map_declaration "packet_count" U32 U64 HashMap config true ~is_pinned:false pos in
  let byte_count_map = make_ast_map_declaration "byte_count" U32 U64 HashMap config true ~is_pinned:false pos in
...
let test_map_type_compatibility () =
  ...
  let compatible_map = make_ast_map_declaration "compatible" U32 U64 HashMap config true ~is_pinned:false pos in
  check string "compatible map name" "compatible" compatible_map.name;
  check bool "compatible types" true (compatible_map.map_type = HashMap)
...
let test_map_size_validation () =
  ...
  List.iter (fun size ->
    ...
    let map_decl = make_ast_map_declaration "test" U32 U64 HashMap config true ~is_pinned:false pos in
...
let test_map_error_handling () =
  ...
  try
    let _ = make_ast_map_declaration "" U32 U64 HashMap config true ~is_pinned:false pos in
...
let test_map_metadata () =
  let pos = make_position 1 1 "test.ks" in
  let config = make_map_config 1024 () in
  let map_decl = make_ast_map_declaration "metadata_test" U32 U64 HashMap config true ~is_pinned:false pos in
...
let test_map_serialization () =
  let pos = make_position 1 1 "test.ks" in
  let config = make_map_config 1024 () in
  let original_map = make_ast_map_declaration "serialize_test" U32 U64 HashMap config true ~is_pinned:false pos in
