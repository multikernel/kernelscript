...
let _test_analyze_ir_function _ =
  ...
  let const_100 = make_simple_ir_value (IRLiteral (IntLit (Signed64 100L, None))) IRU32 in
  let const_2 = make_simple_ir_value (IRLiteral (IntLit (Signed64 2L, None))) IRU32 in
...
let test_variable_liveness_analysis () =
  try
    ...
    let var_b = make_simple_ir_value (IRVariable "b") IRU32 in
    let const_10 = make_simple_ir_value (IRLiteral (IntLit (Signed64 10L, None))) IRU32 in
    let const_20 = make_simple_ir_value (IRLiteral (IntLit (Signed64 20L, None))) IRU32 in
...
let test_function_call_analysis () =
  try
    ...
    let return_instr = make_simple_instruction (IRReturn (Some (make_simple_ir_value (IRLiteral (IntLit (Signed64 0L, None))) IRU32))) in
...
let test_memory_access_analysis () =
  try
    ...
    let _offset = make_simple_ir_value (IRLiteral (IntLit (Signed64 14L, None))) IRU32 in
    ...
    let return_instr = make_simple_instruction (IRReturn (Some (make_simple_ir_value (IRLiteral (IntLit (Signed64 2L, None))) IRU32))) in
...
let test_safety_violations_detection () =
  try
    ...
    let bounds = {min_size = None; max_size = None; alignment = 1; nullable = false} in
    let data_ptr = make_simple_ir_value (IRVariable "data") (IRPointer (IRU8, bounds)) in
    let _unchecked_offset = make_simple_ir_value (IRLiteral (IntLit (Signed64 100L, None))) IRU32 in
    ...
    let return_instr = make_simple_instruction (IRReturn (Some (make_simple_ir_value (IRLiteral (IntLit (Signed64 1L, None))) IRU32))) in
...
let test_complexity_analysis () =
  try
    ...
    let _const_0 = make_simple_ir_value (IRLiteral (IntLit (Signed64 0L, None))) IRU32 in
