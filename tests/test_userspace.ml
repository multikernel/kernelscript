...
let test_nested_userspace_disallowed () =
  ...
  | _ ->
      Printf.printf "✗ FAIL: Nested userspace disallowed (unexpected error)\n"
...
let test_userspace_main_correct_signature () =
  ...
  try
    ...
    let _ = generate_ir ast symbol_table in
    Printf.printf "✓ PASS: Correct userspace main signature\n"
  with
  | Parse_error (_, _) -> 
      Printf.printf "✗ FAIL: Correct userspace main signature (parse error)\n"
  | Failure _ ->
      Printf.printf "✗ FAIL: Correct userspace main signature (validation error)\n"
  | _ ->
      Printf.printf "✗ FAIL: Correct userspace main signature (unexpected error)\n"
...
let test_userspace_main_wrong_return_type () =
  ...
  | e ->
      Printf.printf "✗ Wrong return type test failed: unexpected error: %s\n" (Printexc.to_string e)
...
let test_userspace_main_too_few_params () =
  ...
  | e ->
      Printf.printf "✗ Too few parameters test failed: unexpected error: %s\n" (Printexc.to_string e)
...
let test_userspace_main_too_many_params () =
  ...
  | e ->
      Printf.printf "✗ Too many parameters test failed: unexpected error: %s\n" (Printexc.to_string e)
...
let test_userspace_missing_main () =
  ...
  | e ->
      Printf.printf "✗ Missing main function test failed: unexpected error: %s\n" (Printexc.to_string e)
...
let test_userspace_with_other_functions () =
  ...
  with
  | Parse_error (msg, pos) -> 
      Printf.printf "✗ Userspace with other functions test failed: parse error: %s\n" (string_of_parse_error (msg, pos))
  | Failure msg ->
      Printf.printf "✗ Userspace with other functions test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace with other functions test failed: %s\n" (Printexc.to_string e)
...
let test_userspace_with_structs () =
  ...
  | Failure msg ->
      Printf.printf "✗ Userspace with structs test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace with structs test failed: %s\n" (Printexc.to_string e)
...
let test_multiple_programs_single_userspace () =
  ...
  | Failure msg ->
      Printf.printf "✗ Multiple programs with single userspace test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Multiple programs with single userspace test failed: %s\n" (Printexc.to_string e)
...
let test_userspace_codegen_main_signature () =
  ...
  | Failure msg ->
      Printf.printf "✗ Userspace codegen main signature test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace codegen main signature test failed: %s\n" (Printexc.to_string e);
...
let test_userspace_codegen_file_naming () =
  ...
  | Failure msg ->
      Printf.printf "✗ Userspace codegen file naming test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace codegen file naming test failed: %s\n" (Printexc.to_string e)
...
let test_userspace_codegen_with_structs () =
  ...
  with
  | Parse_error (msg, pos) -> 
      Printf.printf "✗ Userspace codegen with structs test failed: parse error: %s\n" (string_of_parse_error (msg, pos))
  | Failure msg ->
      Printf.printf "✗ Userspace codegen with structs test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace codegen with structs test failed: %s\n" (Printexc.to_string e)
...
let test_userspace_codegen_multiple_functions () =
  ...
  | Failure msg ->
      Printf.printf "✗ Userspace codegen multiple functions test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace codegen multiple functions test failed: %s\n" (Printexc.to_string e)
...
let test_userspace_codegen_includes_structure () =
  ...
  | Failure msg ->
      Printf.printf "✗ Userspace codegen includes and structure test failed: %s\n" msg
  | e ->
      Printf.printf "✗ Userspace codegen includes and structure test failed: %s\n" (Printexc.to_string e)
...
let test_userspace_codegen_error_handling () =
  ...
  | e ->
      Printf.printf "✗ Userspace codegen error handling test failed: unexpected error: %s\n" (Printexc.to_string e)

(** Run all userspace syntax tests *)
let run_all_tests () =
  Printf.printf "Running userspace block syntax and code generation tests...\n\n";
  
  Printf.printf "=== Parser Tests ===\n";
  test_userspace_top_level ();
  test_nested_userspace_disallowed ();
  
  Printf.printf "\n=== Main Function Signature Tests ===\n";
  test_userspace_main_correct_signature ();
  test_userspace_main_wrong_param_types ();
  test_userspace_main_wrong_return_type ();
  test_userspace_main_too_few_params ();
  test_userspace_main_too_many_params ();
  
  Printf.printf "\n=== Main Function Existence Tests ===\n";
  test_userspace_missing_main ();
  test_userspace_multiple_main ();
  
  Printf.printf "\n=== Integration Tests ===\n";
  test_userspace_with_other_functions ();
  test_userspace_with_structs ();
  test_multiple_programs_single_userspace ();
  
  Printf.printf "\n=== Code Generation Tests ===\n";
  test_userspace_codegen_main_signature ();
  test_userspace_codegen_file_naming ();
  test_userspace_codegen_with_structs ();
  test_userspace_codegen_multiple_functions ();
  test_userspace_codegen_includes_structure ();
  test_userspace_codegen_error_handling ();
  
  Printf.printf "\n✅ All userspace syntax and code generation tests completed!\n"

(** Main entry point *)
let () = 
  (* Note: To run with clean output, use: dune exec tests/test_userspace.exe 2>/dev/null *)
  run_all_tests ()