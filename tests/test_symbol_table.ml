...
let test_type_definition_handling () =
  ...
  let struct_def = StructDef ("PacketInfo", [("size", U32); ("protocol", U16)], { line = 1; column = 1; filename = "test" }) in
  add_type_def table struct_def;
  
  (* Add enum definition *)
  let enum_def = EnumDef ("TestEnum", [("Value1", Some (Signed64 0L)); ("Value2", Some (Signed64 1L))], { line = 1; column = 1; filename = "test" }) in
  add_type_def table enum_def;
...
let test_complex_integration () =
  ...
  let enum_def = EnumDef ("xdp_action", [("XDP_PASS", Some (Signed64 2L)); ("XDP_DROP", Some (Signed64 1L))], { line = 1; column = 1; filename = "test" }) in
  add_type_def table enum_def;
