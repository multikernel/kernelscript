             ...
             (match arr_expr.expr_type with
              ...
              | Some (Ast.Str size) ->
                  ...
                  if Int64.compare idx_int64 (Int64.of_int size) >= 0 || Int64.compare idx_int64 0L < 0 then
                    ...
                    errors
              | Some _ ->
                  (* Other types don't need bounds checking *)
                  errors
              | None -> 
                  (* Type annotation missing - this shouldn't happen with type-annotated AST *)
         ...
         | Identifier arr_name, _ ->
             ...
             (match arr_expr.expr_type with
              ...
              | Some (Ast.Array (_, _)) ->
                  ...
                  UnknownBounds arr_name :: errors
              | Some (Ast.Str _) ->
                  (* String character access with dynamic index - safe in kernelscript *)
                  errors
              | Some _ ->
                  (* Other types don't need bounds checking *)
                  errors
